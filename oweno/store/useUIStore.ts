...
const savedAuth = typeof localStorage !== 'undefined' ? localStorage.getItem('oweno_auth') === 'true' : false;
const savedUser = typeof localStorage !== 'undefined' ? JSON.parse(localStorage.getItem('oweno_user') || 'null') : null;
...
export const useUIStore = create<UIState>((set) => ({
  ...
  updateUser: (user) => set((state) => {
    ...
    localStorage.setItem('oweno_user', JSON.stringify(updatedUser));
  ...
  login: (email: string, name?: string) => {
    ...
    localStorage.setItem('oweno_auth', 'true');
    localStorage.setItem('oweno_user', JSON.stringify(user));
    set({ isAuthenticated: true, currentUser: user });
  },

  logout: () => {
    localStorage.removeItem('oweno_auth');
    localStorage.removeItem('oweno_user');
