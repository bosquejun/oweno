						...
						{groups.map((group) => {
							...
							return (
								...
								>
									...
									<h3 className="text-xl font-black text-slate-900 mb-1 group-hover:text-emerald-600 transition-colors truncate">
										{group.name}
									</h3>
									<p className="text-xs text-slate-500 mb-6 line-clamp-2 h-8 font-medium">
										{group.description || "Just a fun group spending together!"}
									</p>
										...
										<div className="flex -space-x-2.5">
											{group.members.slice(0, 4).map((m) => (
												<img
													...
													src={
														m.avatar ||
														`https://api.dicebear.com/7.x/avataaars/svg?seed=${m?.clerkId}&backgroundColor=b6e3f4`
													}
											...
											{group.members.length > 4 && (
												<div className="w-8 h-8 rounded-full border-2 border-white bg-emerald-50 flex items-center justify-center text-[10px] font-black text-emerald-700 shadow-sm">
													+{group.members.length - 4}
												</div>
											)}
										</div>
						...
						{groups.length === 0 && (
							<div className="col-span-full py-16 bg-white rounded-[2rem] border-2 border-dashed border-slate-100 flex flex-col items-center justify-center text-center px-4">
								...
								<p className="text-xs text-slate-500 font-medium">
									Create a group to track your next adventure!
								</p>
							</div>
						)}
					</div>
