...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
		...
		return (
			<div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-6">
				...
				<p className="text-sm text-slate-500 font-medium mb-8 max-w-xs">
					We couldn't find the group with ID: <span className="font-bold text-slate-700">{id}</span>
					. Maybe it was deleted or the link is wrong.
				</p>
				<Link
					href="/groups"
					className="px-8 py-3 bg-emerald-600 text-white rounded-2xl font-black shadow-xl hover:bg-emerald-700 transition-all active:scale-95 text-xs uppercase tracking-widest"
				>
					Back to My Groups
				</Link>
			</div>
	...
	return (
		<ClientPage friends={friends.data || []} group={group} user={user} expenses={expenses || []} />
	);
}
