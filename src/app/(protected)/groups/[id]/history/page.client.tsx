			...
			<header className="sticky top-0 z-40 -mx-6 px-6 pt-8 pb-4 bg-[#F8FAFC]/90 backdrop-blur-xl mb-4">
				<div className="flex items-center gap-4 mb-6">
					<Link
						...
						className="p-2 -ml-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-xl transition-all"
					>
						<ArrowLeft size={22} />
					</Link>
					<div>
				...
				{groupedExpenses.length === 0 ? (
					<div className="text-center py-20 bg-white rounded-[2.5rem] border-2 border-dashed border-slate-100 flex flex-col items-center justify-center">
						...
						<p className="text-xs text-slate-400 font-bold uppercase tracking-tight">
							Try adjusting your filters, bes!
						</p>
					</div>
				) : (
							...
							<div className="space-y-3">
								{mGroup.items.map((expense) => {
									...
									return (
										<div
											key={expense.id}
											className="group bg-white rounded-3xl p-5 border border-slate-50 shadow-sm hover:border-emerald-200 transition-all"
										>
											<div className="flex items-center gap-4">
												<div
													className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${style.color}`}
												>
													<Icon size={20} />
												</div>
												<div className="flex-1 min-w-0">
													...
													<div className="flex items-center gap-2 mt-0.5">
														...
														<span className="text-[9px] font-black text-emerald-600 uppercase tracking-tight">
															Paid by {paidByMember?.displayName || "Someone"}
														</span>
													</div>
												</div>
												<div className="flex items-center gap-3">
													<div className="text-right">
														<p
															className={`text-base font-black tabular-nums ${expense.category === "Settlement" ? "text-rose-500" : "text-slate-900"}`}
														>
