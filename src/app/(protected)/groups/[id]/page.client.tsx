				...
				<header className="sticky top-0 z-30 -mx-6 px-6 pt-8 pb-4 bg-[#F8FAFC]/90 backdrop-blur-xl flex flex-row items-center justify-between gap-4 mb-6 md:mb-10">
							...
							<div className="overflow-hidden">
								...
								<div className="flex items-center gap-2 mt-1">
									<button
										...
										className="text-[9px] md:text-[10px] text-emerald-600 font-black uppercase tracking-widest hover:underline"
									>
										Edit Group
									</button>
					...
					<div className="flex items-center gap-2">
						...
						<button
							...
							className="p-3 md:px-6 md:py-3 bg-slate-900 text-white rounded-2xl font-black active:scale-95 transition-all shadow-xl shrink-0"
						>
							<Plus size={20} />
						</button>
					</div>
					...
					<div className="lg:col-span-8 space-y-6">
						<div className="bg-emerald-600 rounded-[2.5rem] p-8 text-white flex items-center justify-between shadow-xl relative overflow-hidden">
							...
							<div className="relative z-10">
								<span className="text-emerald-200 text-[9px] font-black uppercase tracking-widest">
									Net Standing
								</span>
								...
								<div className="space-y-3 relative z-10">
									{debts.map((debt, idx) => {
										...
										return (
											<div
												key={idx}
												className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 p-4 bg-white/80 backdrop-blur-sm rounded-2xl border border-amber-100 shadow-sm"
											>
												<div className="flex items-center gap-3 min-w-0 flex-1">
													<Image
														...
														src={
															debtor?.avatar ||
															`https://api.dicebear.com/7.x/avataaars/svg?seed=${debtor?.clerkId}&backgroundColor=b6e3f4`
														}
														className="w-8 h-8 rounded-lg border border-slate-100 shrink-0"
														alt=""
														width={32}
														height={32}
													/>
													<div className="flex items-center gap-2 min-w-0 flex-1">
														<span
															className={`text-[11px] font-black truncate ${isMyDebt ? "text-rose-600" : "text-slate-700"}`}
														>
															{isMyDebt ? "You" : debtor?.displayName}
														</span>
														<ArrowRight size={12} className="text-amber-400 shrink-0" />
														<span
															className={`text-[11px] font-black truncate ${isMyCredit ? "text-emerald-600" : "text-slate-700"}`}
														>
						...
						<section className="bg-white rounded-[2.5rem] p-6 md:p-8 shadow-sm border border-slate-100">
							<div className="flex items-center justify-between mb-8">
								...
								<Link
									...
									className="flex items-center gap-2 text-[10px] font-black text-emerald-600 uppercase tracking-widest hover:bg-emerald-50 px-3 py-2 rounded-xl transition-all"
								>
									<History size={14} />
									History
								</Link>
							...
							<div className="space-y-3 md:space-y-4">
								{expenses.length === 0 ? (
									<div className="text-center py-20 bg-slate-50/50 rounded-[2.5rem] border-2 border-dashed border-slate-100">
										...
										<p className="text-xs text-slate-400 font-bold mt-2">
											Start by adding a bill above, lods!
										</p>
									</div>
								) : (
									[...expenses].slice(0, 10).map((expense) => {
										...
										return (
											<div
												key={expense.id}
												className="group relative flex items-center gap-4 p-4 md:p-5 bg-white hover:bg-slate-50 transition-all rounded-[1.5rem] md:rounded-[2rem] border-2 border-slate-200 hover:border-emerald-200"
											>
												<div className="w-12 md:w-14 h-12 md:h-14 bg-slate-50 rounded-2xl flex flex-col items-center justify-center shrink-0 border border-slate-100">
													<span className="text-[8px] md:text-[9px] font-black uppercase text-slate-400 leading-none">
														{format(expense.date, "MMM")}
													</span>
													<span className="text-lg md:text-xl font-black text-slate-900 leading-none mt-0.5">
														{format(expense.date, "dd")}
													</span>
												</div>

												<div className="flex-1 min-w-0">
													<h4 className="font-black text-slate-900 truncate text-sm md:text-base leading-tight">
														{expense.title}
													</h4>
													<div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-1.5">
														<span
															className={`inline-flex items-center gap-1 text-[8px] px-2.5 py-1 rounded-lg font-black uppercase border border-current opacity-70 ${style.color}`}
														>
															<Icon size={10} />
															{expense.category}
														</span>
												...
												<div className="flex items-center gap-2 md:gap-3 shrink-0">
													<p
														className={`text-base md:text-lg font-black tabular-nums ${expense.category === "Settlement" ? "text-rose-500" : "text-slate-900"}`}
													>
								...
								{expenses.length > 10 && (
									<Link
										href={`/groups/${group.id}/history`}
										className="block text-center py-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] hover:text-emerald-600 transition-colors"
									>
									...
									<div key={member.id} className="flex items-center justify-between gap-3">
										<div className="flex items-center gap-3 min-w-0">
											<Image
												...
												src={
													member.avatar ||
													`https://api.dicebear.com/7.x/avataaars/svg?seed=${member.clerkId}&backgroundColor=b6e3f4`
												}
												className="w-9 h-9 rounded-xl border border-slate-100 shadow-sm shrink-0"
												alt=""
												width={36}
												height={36}
											/>
