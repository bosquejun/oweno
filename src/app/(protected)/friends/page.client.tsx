...
export default function FriendsList({
	...
	const filteredFriends = useMemo(() => {
		...
		return friends.filter(
			(f) => f.displayName.toLowerCase().includes(query) || f.email.toLowerCase().includes(query)
		);
	}, [friends, searchQuery]);
		...
		<div className="page-transition">
			<header className="sticky top-0 z-30 -mx-6 px-6 pt-8 pb-4 bg-[#F8FAFC]/90 backdrop-blur-xl flex items-center justify-between gap-3 mb-8 md:mb-10">
				<div>
					...
					<p className="text-[10px] md:text-sm text-slate-500 font-medium truncate uppercase tracking-widest">
						Manage your close circle.
					</p>
				</div>
						...
						<div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
							<div>
								...
								<p className="text-emerald-100 text-xs font-bold">
									Invite a friend to OweNah by their email.
								</p>
							</div>
							...
							>
								<div className="relative mb-4">
									<Image
										...
										src={
											friend.avatar ||
											`https://api.dicebear.com/7.x/avataaars/svg?seed=${friend?.clerkId}&backgroundColor=b6e3f4`
										}
										className="w-20 h-20 rounded-[2rem] border-4 border-slate-50 group-hover:border-emerald-100 transition-all shadow-md"
								...
								<h3 className="text-lg font-black text-slate-900 truncate w-full px-2">
									{friend.displayName}
								</h3>
						...
						{filteredFriends.length === 0 && (
							<div className="col-span-full py-16 bg-white rounded-[2rem] border-2 border-dashed border-slate-100 flex flex-col items-center justify-center text-center px-4">
								...
								<p className="text-xs text-slate-500 font-medium">
									Add friends from the Invited tab or from group details!
								</p>
							</div>
						)}
					</div>
					...
					<div className="space-y-4">
												...
												<div className="flex-1 min-w-0">
													...
													{invite.group && (
														<div className="text-xs text-slate-500 font-medium mb-2">
															For:{" "}
															<span className="font-black text-slate-700">{invite.group.name}</span>
														</div>
													)}
						...
						{filteredInvites.length === 0 && (
							<div className="py-16 bg-white rounded-[2rem] border-2 border-dashed border-slate-100 flex flex-col items-center justify-center text-center px-4">
								...
								<p className="text-xs text-slate-500 font-medium">
									Invite friends using the form above!
								</p>
							</div>
						)}
					</div>
