		...
		return (
			<div className="min-h-screen flex items-center justify-center bg-[#F8FAFC] p-6">
				<div className="bg-white rounded-3xl p-8 max-w-md w-full text-center border border-slate-100 shadow-xl">
					...
					<h1 className="text-2xl font-black text-slate-900 mb-4">Welcome to OweNah! 🎉</h1>
					<p className="text-slate-600 mb-6">
						{invite.groupId
						...
						{user?.emailAddresses[0]?.emailAddress.toLowerCase() === invite.email.toLowerCase() ? (
							...
							<div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
								<p className="text-amber-700 text-sm font-medium text-center">
									...
									as <strong>{user?.emailAddresses[0]?.emailAddress}</strong>.
								</p>
							</div>
						)}
						<SignOutButton>
							<button className="w-full py-3 border-2 border-slate-200 text-slate-700 rounded-2xl font-black hover:bg-slate-50 transition-all text-center">
								Sign out
							</button>
						</SignOutButton>
						{/* <Link
