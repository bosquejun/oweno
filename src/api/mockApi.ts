...
const getStoredData = () => {
  try {
    const data = typeof window !== 'undefined' ? window.localStorage.getItem(STORAGE_KEY) : null;
...
const saveData = (groups: Group[], expenses: Expense[], friends: User[]) => {
  try {
    if (typeof window !== 'undefined') {
      window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ groups, expenses, friends }));
    }
  } catch (e) {
...
// Initialize seed on module load (client-side only)
if (typeof window !== 'undefined') {
  initSeed();
}
