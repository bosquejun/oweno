						...
						<section className="space-y-4 pb-10" ref={friendPickerRef}>
							<div className="flex items-center justify-between">
								<label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">
									Barkada Members ({members.length})
								</label>
							</div>
							...
							<div className="flex flex-wrap gap-2 mb-4">
								{members.map((m) => (
									<div
										key={m.id}
										className="flex items-center gap-2.5 bg-emerald-50/80 px-4 py-2 rounded-xl text-[10px] font-black text-emerald-700 border border-emerald-100 shadow-sm animate-in zoom-in-95"
									>
										<img
							...
							<div className="relative">
								<div className="flex gap-2">
									<div className="flex-1 relative">
										<Search
											size={18}
											className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${isFriendPickerOpen ? "text-emerald-500" : "text-slate-300"}`}
										/>
										<input
											type="text"
								...
								{isFriendPickerOpen && (memberSearchQuery.length > 0 || friends.length > 0) && (
									<div className="absolute top-full mt-3 left-0 right-0 bg-white rounded-[1.5rem] shadow-2xl border border-slate-100 z-[10001] overflow-hidden flex flex-col max-h-64 animate-in slide-in-from-top-2">
										<div className="flex-1 overflow-y-auto p-2 no-scrollbar">
											<p className="px-3 py-2 text-[8px] font-black text-slate-400 uppercase tracking-widest">
												Select from Barkada
											</p>
					...
					<div className="p-6 md:p-10 bg-white border-t border-slate-50 shrink-0 pb-8 md:pb-12 shadow-[0_-15px_40px_rgba(0,0,0,0.04)]">
						<div className="flex gap-4">
							...
							<button
								...
								className="flex-1 px-4 py-4 bg-emerald-600 text-white rounded-[1.5rem] font-black hover:bg-emerald-700 shadow-2xl shadow-emerald-100 transition-all active:scale-95 text-[10px] uppercase tracking-widest disabled:opacity-50"
							>
