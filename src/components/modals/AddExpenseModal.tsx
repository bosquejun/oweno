...
export const AddExpenseModal: React.FC<AddExpenseModalProps> = ({
		...
		if (isOpen) {
			if (initialExpense) {
				...
				initialExpense.splits.forEach((s) => {
					...
					percs[s.userId] =
						metadata[s.userId] || ((s.amount / initialExpense.amount) * 100).toFixed(1);
	...
	const onFormSubmit = async (formData: ExpenseFormData) => {
		...
		if (splitType === SplitType.EQUAL) {
			...
			if (isMismatch) {
				setError("root", {
					message: `Mismatch: Total is ${formatCurrency(numAmount, user.preferredCurrency, user.preferredLocale)}`,
				});
				return;
			}
			...
			splits = involved.map((m, i) => {
				...
				let share =
					i === involved.length - 1
						? Number((numAmount - runningTotal).toFixed(2))
						: Number((numAmount * ((parseFloat(percentages[m.id]) || 0) / 100)).toFixed(2));
				runningTotal += share;
			...
			splits = involved.map((m, i) => {
				...
				let share =
					i === involved.length - 1
						? Number((numAmount - runningTotal).toFixed(2))
						: Number(
								(numAmount * ((parseFloat(shares[m.id]) || 0) / totals.totalShares)).toFixed(2)
							);
				runningTotal += share;
				return { userId: m.id, amount: share };
			});
				...
				<div className="flex items-center justify-between p-6 md:p-10 border-b border-slate-50 shrink-0">
					<div>
						<h2 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight">
							{isEditing ? "Edit Bill ✏️" : "Add Bill 💸"}
						</h2>
						<p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.1em] mt-1">
							Track and split right.
						</p>
					</div>
					<button
						onClick={onClose}
						type="button"
						className="text-slate-300 hover:text-slate-600 p-2.5 hover:bg-slate-50 rounded-2xl transition-all"
					>
						<X size={24} />
					</button>
				</div>
					...
					<div className="flex-1 overflow-y-auto p-6 md:p-10 pt-4 space-y-9 md:space-y-12 no-scrollbar">
						<section className="space-y-4">
							<div className="flex items-center gap-2.5">
								<div className="w-6 h-6 rounded-full bg-emerald-600/10 text-emerald-600 flex items-center justify-center text-[10px] font-black">
									1
								</div>
							...
							<div className="flex flex-wrap gap-2">
								{PRESET_CATEGORIES.map((cat) => (
									<button
										...
										className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border-2 transition-all ${category === cat.name ? `bg-emerald-600 border-emerald-600 text-white shadow-xl shadow-emerald-100` : "bg-white border-slate-50 text-slate-400 hover:border-emerald-200"}`}
									>
										<cat.icon size={14} />
						...
						<section className="space-y-4">
							...
							<div className="relative" ref={pickerRef}>
								<button
									...
									className="w-full flex items-center justify-between p-4 bg-slate-900 text-white rounded-[1.5rem] shadow-xl active:scale-[0.98] transition-all"
								>
									<div className="flex items-center gap-3.5">
										...
										<div className="text-left">
											<p className="text-[11px] font-black uppercase tracking-widest text-white/50 mb-0.5">
												Settled By
											</p>
											<p className="text-sm font-black">
												{paidBy === user.id ? "You" : selectedUser?.displayName}
											</p>
										</div>
									</div>
									<ChevronDown
										size={18}
										className={`text-white/40 transition-transform ${isUserPickerOpen ? "rotate-180" : ""}`}
									/>
								</button>
								{isUserPickerOpen && (
										...
										<div className="p-3 border-b border-slate-50 bg-slate-50/50">
											<div className="relative">
												...
												<input
													...
													className="w-full bg-white border border-slate-100 rounded-xl pl-9 pr-4 py-2 text-xs font-bold text-slate-900 placeholder:text-slate-400 outline-none focus:border-emerald-500 transition-all"
												/>
											</div>
										</div>
											...
											{filteredMembers.length > 0 ? (
												filteredMembers.map((m) => (
													<button
														...
														className={`w-full flex items-center justify-between p-3 rounded-xl transition-all ${paidBy === m.id ? "bg-emerald-50 text-emerald-700" : "hover:bg-slate-50"}`}
													>
						...
						<section className="space-y-4 pb-12">
							...
							<div className="bg-slate-100/80 p-1.5 rounded-2xl flex gap-1.5">
								{[
									...
									<button
										key={strat.id}
										type="button"
										onClick={() => setSplitType(strat.id as SplitType)}
										className={`flex-1 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${splitType === strat.id ? "bg-white text-emerald-600 shadow-lg shadow-black/5" : "text-slate-400"}`}
									>
										{strat.label}
									</button>
								))}
							...
							{(splitType === SplitType.EXACT || splitType === SplitType.PERCENT) &&
								numAmount > 0 && (
									<div
										className={`p-4 rounded-2xl border-2 flex items-center justify-between transition-all animate-in slide-in-from-top-2 ${isMismatch ? "bg-rose-50 border-rose-100 text-rose-700" : "bg-emerald-50 border-emerald-100 text-emerald-700"}`}
									>
										<div className="flex items-center gap-2.5">
											{isMismatch ? (
							...
							<div className="space-y-2.5">
								{group.members.map((member) => {
									...
									if (splitType === SplitType.EQUAL)
										calculatedAmount = numAmount / (involvedUserIds.size || 1);
									else if (splitType === SplitType.EXACT)
										calculatedAmount = parseFloat(exactAmounts[member.id]) || 0;
									else if (splitType === SplitType.PERCENT)
										calculatedAmount =
											numAmount * ((parseFloat(percentages[member.id]) || 0) / 100);
									else if (splitType === SplitType.SHARES)
										calculatedAmount =
											numAmount *
											((parseFloat(shares[member.id]) || 0) / (totals.totalShares || 1));

									return (
										<div
											key={member.id}
											className={`flex items-center gap-4 p-4 rounded-[1.5rem] border-2 transition-all ${isInvolved ? "bg-white border-emerald-100 shadow-sm" : "bg-slate-50/30 border-transparent opacity-40"}`}
										>
											<button
												type="button"
												onClick={() => toggleUserInvolvement(member.id)}
											...
											>
												<Image
													...
													height={40}
													src={member.avatar || ""}
													className={`w-10 h-10 rounded-xl border-2 transition-all ${isInvolved ? "border-emerald-500 scale-105 shadow-md shadow-emerald-50" : "border-slate-200"}`}
											...
											{isInvolved && splitType !== SplitType.EQUAL && (
												<div className="relative">
													<input
														...
														className="w-20 px-3 py-2 bg-slate-50 border border-slate-100 rounded-xl text-xs font-black text-slate-900 placeholder:text-slate-400 text-right outline-none focus:border-emerald-500 focus:bg-white transition-all"
													/>
												</div>
											)}
										</div>
							...
							{errors.root && (
								<p className="text-[10px] text-rose-500 font-black text-center mt-4 uppercase tracking-widest bg-rose-50 p-3 rounded-xl border border-rose-100">
									{errors.root.message}
								</p>
							)}
						</section>
					...
					<div className="p-6 md:p-10 bg-white border-t border-slate-50 shrink-0 pb-8 md:pb-12 shadow-[0_-15px_40px_rgba(0,0,0,0.04)]">
						<div className="flex gap-4">
							<button
								type="button"
								onClick={onClose}
								className="flex-1 px-6 py-4 border-2 border-slate-100 rounded-[1.5rem] font-black text-slate-400 uppercase tracking-widest text-[10px] transition-all active:scale-95 hover:bg-slate-50"
							>
								Cancel
							</button>
							<button
								...
								className="flex-1 px-6 py-4 bg-emerald-600 text-white rounded-[1.5rem] font-black shadow-2xl shadow-emerald-100 uppercase tracking-widest text-[10px] transition-all active:scale-95 hover:bg-emerald-700 disabled:opacity-50 disabled:grayscale"
							>
