...
export const SettleDebtModal: React.FC<SettleDebtModalProps> = ({
	...
	const onFormSubmit = async (formData: SettleFormData) => {
		const settlementExpense: Expense = {
			...
			category: "Settlement",
			splits: [{ userId: debt.to, amount: formData.amount }],
		...
		<div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-end md:items-center justify-center z-[9999] p-0 md:p-6">
			<div className="bg-white rounded-t-[2.5rem] md:rounded-[3rem] w-full md:max-w-md h-auto md:max-h-[85vh] overflow-hidden flex flex-col shadow-2xl animate-in slide-in-from-bottom-5 duration-300">
				<header className="flex items-center justify-between p-6 md:p-10 border-b border-slate-50 bg-amber-50/20 shrink-0">
					<div>
						<h2 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight">
							{expenseId ? "Edit Settlement 💰" : "Settle Debt 💰"}
						</h2>
						<p className="text-[10px] font-black text-amber-600 uppercase tracking-widest mt-1">
							{expenseId ? "Update payment record" : "Record a payment"}
						</p>
					</div>
					<button
						onClick={onClose}
				...
				<form
					...
					className="p-6 md:p-10 space-y-8 flex-1 overflow-y-auto no-scrollbar"
				>
					<div className="flex items-center justify-center gap-6 py-6">
						<div className="flex flex-col items-center gap-3">
							<Image
								...
								height={64}
								src={debtor?.avatar || ""}
								className="w-16 h-16 md:w-20 md:h-20 rounded-[1.5rem] border-4 border-amber-100 shadow-lg"
								alt=""
							/>
						...
						<div className="flex flex-col items-center gap-3">
							<Image
								...
								height={64}
								src={creditor?.avatar || ""}
								className="w-16 h-16 md:w-20 md:h-20 rounded-[1.5rem] border-4 border-emerald-100 shadow-lg"
								alt=""
							/>
							<p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
					...
					<div className="bg-slate-50 p-5 rounded-[1.5rem] border border-slate-100">
						<p className="text-[10px] md:text-xs font-bold text-slate-500 leading-relaxed text-center px-2">
							This records that{" "}
							<span className="text-slate-900 font-black">{debtor?.displayName}</span> paid{" "}
							<span className="text-slate-900 font-black">{creditor?.displayName}</span> directly.
							This balances your books instantly!
						</p>
					</div>
					...
					<div className="flex gap-4 pt-4">
						<button
							type="button"
							onClick={onClose}
							className="flex-1 px-4 py-4 border-2 border-slate-100 rounded-2xl font-black text-slate-400 uppercase tracking-widest text-[10px] transition-all hover:bg-slate-50"
						>
							Cancel
						</button>
						<button
