        ...
        <div className="mt-auto space-y-6 pt-10">
          ...
          >
            <div className="relative shrink-0">
              <img src={user.imageUrl} alt={user.emailAddresses[0].emailAddress} className="w-12 h-12 rounded-[1.1rem] border-2 border-white shadow-md group-hover/profile:scale-105 transition-transform" />
              <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-emerald-500 border-2 border-white rounded-full shadow-sm"></div>
            </div>
            <div className="flex-1 min-w-0 text-left">
              <p className="text-sm font-black text-slate-900 truncate">{user.firstName || user.emailAddresses[0].emailAddress}</p>
          ...
          >
             {isSigningOut ? <Loader2 size={16} strokeWidth={3} className="animate-spin" /> : <LogOut size={16} strokeWidth={3} />}
